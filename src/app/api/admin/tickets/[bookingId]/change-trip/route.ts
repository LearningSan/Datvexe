...
export async function POST(req: NextRequest, context: Context) {
  try {
    ...
    const data = await changeAdminTicketTrip(bookingId, parsed);

    return successResponse(data, "Đổi vé thành công");
  } catch (error: unknown) {
    console.error("[ADMIN CHANGE TICKET ERROR]", error);

    const message = error instanceof Error ? error.message : "Không thể đổi vé";
