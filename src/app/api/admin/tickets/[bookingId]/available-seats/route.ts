...
export async function GET(req: NextRequest, context: Context) {
  try {
    ...
    const data = await getAvailableSeatsForAdminTicket(bookingId);

    return successResponse(data);
  } catch (error: unknown) {
    console.error("[ADMIN AVAILABLE SEATS ERROR]", error);

    const message =
      error instanceof Error ? error.message : "Không thể tải ghế khả dụng";
