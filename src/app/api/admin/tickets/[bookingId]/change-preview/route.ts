...
export async function GET(req: NextRequest, context: Context) {
  try {
    ...
    const data = await getChangeAdminTicketPreview(bookingId, parsed);

    return successResponse(data);
  } catch (error: unknown) {
    console.error("[ADMIN CHANGE PREVIEW ERROR]", error);

    const message =
      error instanceof Error ? error.message : "Không thể xem trước đổi vé";
