...
export async function POST(req: NextRequest, context: Context) {
  try {
    ...
    const data = await undoCheckinAdminTicket(bookingId);

    return successResponse(data, "Đã bỏ check-in booking");
  } catch (error: unknown) {
    console.error("[ADMIN UNDO CHECKIN BOOKING ERROR]", error);

    const message =
      error instanceof Error ? error.message : "Không thể bỏ check-in booking";
