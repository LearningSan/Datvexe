...
export default function ScheduleTemplateFormModal({
  ...
  const routeEstimatedDuration =
    routeOptions.find((r) => r.routeId === Number(routeId))
      ?.estimatedDuration ??
    schedule?.estimatedDuration ??
    0;
  const durationDiff = Number(estimatedDuration) - routeEstimatedDuration;
  // Tìm tên tuyến xe đang chọn để hiển thị tiêu đề nhắc nhở trực quan
  const selectedRoute = useMemo(() => {
    return routeOptions.find((route) => route.routeId === Number(routeId));
  }, [routeOptions, routeId]);

  const selectedRouteName =
    selectedRoute?.routeName ?? schedule?.routeName ?? "";
  ...
  useEffect(() => {
    ...
    setBasePrice(String(selectedRoute.basePrice));
    setEstimatedDuration(String(selectedRoute.estimatedDuration));
  }, [selectedRoute, mode]);
  if (!open) return null;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!routeId) {
      newErrors.routeId = "Vui lòng chọn tuyến đường vận hành.";
    }

    if (!departureTime) {
      newErrors.departureTime = "Vui lòng chọn giờ xuất bến.";
    }
    ...
    if (!estimatedDuration) {
      newErrors.estimatedDuration =
        "Vui lòng nhập thời gian di chuyển dự kiến.";
    } else if (!Number.isInteger(duration) || duration <= 0) {
      newErrors.estimatedDuration =
        "Thời gian di chuyển phải là số nguyên lớn hơn 0.";
    }

    const price = Number(basePrice);

    if (!basePrice) {
      newErrors.basePrice = "Vui lòng nhập giá vé cơ bản.";
          ...
          <div className={styles.grid2}>
            ...
            <div className={styles.formGroup}>
              ...
              <div className={styles.routeHint}>
                <div>
                  <strong>Thời gian chuẩn của tuyến:</strong>{" "}
                  {routeEstimatedDuration} phút
                </div>

                {durationDiff !== 0 && (
                  <div className={styles.durationCompare}>
                    {durationDiff > 0
                      ? `Đang dài hơn ${durationDiff} phút so với chuẩn`
                      : `Đang ngắn hơn ${Math.abs(durationDiff)} phút so với chuẩn`}
                  </div>
                )}

                {Number(estimatedDuration) !== routeEstimatedDuration && (
                  <button
                    type="button"
                    className={styles.resetDurationBtn}
                    onClick={() =>
                      setEstimatedDuration(String(routeEstimatedDuration))
                    }
                  >
                    Khôi phục về thời gian chuẩn
                  </button>
                )}
              </div>
              ...
              {durationDiff !== 0 && (
                <div className={styles.durationCompare}>
                  {durationDiff > 0 ? (
                    <>
                      ⏱ Dài hơn chuẩn <strong>{durationDiff} phút</strong>
                    </>
                  ) : (
                    <>
                      ⚡ Ngắn hơn chuẩn{" "}
                      <strong>{Math.abs(durationDiff)} phút</strong>
                    </>
                  )}
                </div>
              )}
